...
public class ParticleUtils {
    ...
    public static void spawnHealParticles(EntityLiving en, int amount) {

        spawnParticles(ParticleTypes.HEART, en, amount);

    }

    public static void spawnEnergyRestoreParticles(EntityPlayer player, int amount) {

        spawnParticles(ParticleTypes.HAPPY_VILLAGER, player, amount);

    }

    public static void spawnManaRestoreParticles(EntityLiving en, int amount) {

        spawnParticles(ParticleTypes.BUBBLE, en, amount);

    }

    public static void spawnParticles(IParticleFactory particle, EntityLiving en,
                                      int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            if (en.world.isRemote) {
                en.world.spawnParticle(particle, true, d0, d1, d2, 0, 48 + en.world.rand.nextInt(12), 1.0f);
            } else {
                ((WorldServer) en.world).spawnParticle(ParticleTypes.HAPPY_VILLAGER, d0, d1, d2, 10, 0.0D, 0.0D, 0.0D, (double) 1F);
            }
